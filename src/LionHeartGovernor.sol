...
/**
 * @title LionHeartGovernor
 * @notice Governance contract for the Lion Heart DAO with token-based voting
 * @dev Implements OpenZeppelin Governor pattern with timelock control
 */
contract LionHeartGovernor is
    Governor,
    GovernorSettings,
    ...
    AccessControl
{
    ...
    /// @dev The LHGT governance token contract
    ...
    error LionHeartGovernor__ZeroTokenAddress_constructor();
    
    /// @dev Error thrown when timelock address is zero
    error LionHeartGovernor__ZeroTimelockAddress_constructor();
     ...
     * @param _governanceToken Address of the LHGT governance token
        ...
        GovernorTimelockControl(_timelock)
    {
        if (address(_governanceToken) == address(0)) {
            revert LionHeartGovernor__ZeroTokenAddress_constructor();
        }
        if (address(_timelock) == address(0)) {
            revert LionHeartGovernor__ZeroTimelockAddress_constructor();
        }
