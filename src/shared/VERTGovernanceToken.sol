 ...
 *
 * This contract implements a governance token for WAGA DAO (Vertical Integration Token - VERT)
 ...
 * - Swiss Verein compliant governance structure for regenerative agriculture
 */
contract VERTGovernanceToken is
    ERC20,
    ERC20Votes,
    ERC20Permit,
    Ownable,
    ...
    Pausable
{
    ...
    error VERTGovernanceToken__AddressNotVerified_mint(address account);

    /// @dev Error thrown when trying to set zero address for identity registry
    error VERTGovernanceToken__ZeroAddressNotAllowed_setIdentityRegistry();

    /// @dev Error thrown when unauthorized minting is attempted
    error VERTGovernanceToken__UnauthorizedMinting_mint(address account);

    /// @dev Error thrown when trying to transfer to/from unverified addresses
    error VERTGovernanceToken__TransferNotAllowed_update(
        address from,
        address to
    );

    /// @dev Error thrown when zero address provided in constructor
    error VERTGovernanceToken__ZeroAddressNotAllowed_constructor();
    ...
    string private constant TOKEN_NAME = "WAGA Vertical Integration Token";

    /// @dev Token symbol (immutable)
    string private constant TOKEN_SYMBOL = "VERT";
        ...
        Ownable(_initialOwner)
    {
        if (_identityRegistry == address(0)) {
            revert VERTGovernanceToken__ZeroAddressNotAllowed_constructor();
        }
        if (_initialOwner == address(0)) {
            revert VERTGovernanceToken__ZeroAddressNotAllowed_constructor();
        }
        // @explanation: s_priceFeed = AggregatorV3Interface(_priceFeed); (We are doing the same thing here)
        ...
        _delegate(_initialOwner, _initialOwner); // @audit: Why are we doing this. 
    ...
    function mint(
        ...
        if (!s_identityRegistry.isVerified(to)) {
            revert VERTGovernanceToken__AddressNotVerified_mint(to);
        }
     ...
     */
     //@audit: Why are we allowing users to burn their tokens? 
     ...
     * @param amount The amount of tokens to burn
     */
     // @audit: Why are we allowing users to burn tokens from other addresses?
    ...
    function setIdentityRegistry(
        ...
        if (_newRegistry == address(0)) {
            revert VERTGovernanceToken__ZeroAddressNotAllowed_setIdentityRegistry();
        }
    ...
    function _update(
        ...
        if (from != address(0) && !s_identityRegistry.isVerified(from)) {
            revert VERTGovernanceToken__TransferNotAllowed_update(from, to);
        }
