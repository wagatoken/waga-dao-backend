...
/**
 * @title LionHeartTimelock
 * @dev Timelock controller for the Lion Heart Football Centre DAO
 ...
 */
contract LionHeartTimelock is TimelockController {
    ...
    error LionHeartTimelock__InvalidDelay_constructor();
    
    /// @dev Error thrown when proposers array is empty
    error LionHeartTimelock__EmptyProposers_constructor();
    ...
    constructor(
        ...
        if (minDelay < MIN_DELAY) {
            revert LionHeartTimelock__InvalidDelay_constructor();
        }
        
        // Validate proposers array
        if (proposers.length == 0) {
            revert LionHeartTimelock__EmptyProposers_constructor();
        }
