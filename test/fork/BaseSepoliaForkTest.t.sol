...
contract BaseSepoliaForkTest is Test {
    ...
    HelperConfig.NetworkConfig memory config = helperConfig.getConfigByChainId(block.chainid);
        ...
        console.log("Greenfield Manager deployed to:", address(greenfieldManager));
    ...
    function _setupRolesAndPermissions() internal {
        ...
        console.log("Granted MINTER_ROLE to DonationHandler");
                ...
                // Grant MINTER_ROLE to GrantManager for Coffee tokens
        bytes32 coffeeTokenMinterRole = coffeeToken.MINTER_ROLE();
        coffeeToken.grantRole(coffeeTokenMinterRole, address(grantManager));
        console.log("Granted MINTER_ROLE to GrantManager for Coffee tokens");
        
        // Grant permissions to governor
        bytes32 grantManagerRole = grantManager.GRANT_MANAGER_ROLE();
        grantManager.grantRole(grantManagerRole, address(governor));
        console.log("Granted GRANT_MANAGER_ROLE to Governor");
        
        // Grant MINTER_ROLE to test accounts for coffee batch creation
        coffeeToken.grantRole(coffeeTokenMinterRole, ALICE);
        coffeeToken.grantRole(coffeeTokenMinterRole, BOB);
        coffeeToken.grantRole(coffeeTokenMinterRole, DAVID);
        
        // Grant DAO_ADMIN_ROLE to test accounts for createBatch function
        bytes32 daoAdminRole = coffeeToken.DAO_ADMIN_ROLE();
        coffeeToken.grantRole(daoAdminRole, ALICE);
        coffeeToken.grantRole(daoAdminRole, BOB);
        coffeeToken.grantRole(daoAdminRole, DAVID);
        console.log("Granted DAO_ADMIN_ROLE to test accounts for createBatch");
        
        // Grant GRANT_MANAGER_ROLE to test accounts for creating grants
        grantManager.grantRole(grantManagerRole, ALICE);
        grantManager.grantRole(grantManagerRole, BOB);
        grantManager.grantRole(grantManagerRole, DAVID);
        console.log("Granted GRANT_MANAGER_ROLE to test accounts");
        
        // Grant PROJECT_MANAGER_ROLE to coffeeToken for greenfield projects
        bytes32 projectManagerRole = greenfieldManager.PROJECT_MANAGER_ROLE();
        greenfieldManager.grantRole(projectManagerRole, address(coffeeToken));
        console.log("Granted PROJECT_MANAGER_ROLE to coffeeToken");
    }
    ...
    function _verifyDeployments() internal view {
        ...
        require(address(grantManager) != address(0), "Grant Manager not deployed");
    ...
    function _setupTestAccounts() internal {
        ...
        deal(USDC_TOKEN, address(grantManager), 100000e6); // Grant pool
    ...
    // Coffee token does not implement name() or symbol(), so we skip those checks
        ...
        assertEq(governor.proposalThreshold(), 100_000e18); // 100K tokens
    ...
    function testCoffeeBatchTokenization() public {
        ...
        uint256 batchId = coffeeToken.createBatch(
            CoffeeStructs.BatchCreationParams({
                productionDate: block.timestamp - 10 days,
                expiryDate: block.timestamp + 180 days,
                quantity: 1000,
                pricePerKg: 5e6,
                grantValue: 2000e6,
                ipfsHash: "ipfs://batch1"
            })
        );
        
    // Verify batch creation
    // Since we have struct name conflicts, let's just verify via individual calls
    uint256 batchQuantity = coffeeToken.getBatchInfo(batchId).currentQuantity;
    uint256 batchPrice = coffeeToken.getBatchInfo(batchId).pricePerKg;
    assertEq(batchQuantity, 1000, "Quantity should match");
    assertEq(batchPrice, 5e6, "Price should match");
    // No direct paymentAddress in BatchInfo, check via cooperativeInfo if needed
        
        console.log("Batch created - ID:", batchId);
        console.log("Quantity:", batchQuantity, "kg");
        console.log("Price per kg:", batchPrice);
    ...
    /**
     * @dev Test 4: Cooperative grant creation and management
     */
    function testCooperativeGrantCreation() public {
        console.log("=== TEST 4: COOPERATIVE GRANT CREATION ===");
        
        // First, create a coffee batch as collateral
        vm.prank(BOB);
        uint256 batchId = coffeeToken.createBatch(
            CoffeeStructs.BatchCreationParams({
                productionDate: block.timestamp - 5 days,
                expiryDate: block.timestamp + 200 days,
                quantity: 500,
                pricePerKg: 6e6,
                grantValue: 1200e6,
                ipfsHash: "ipfs://batch2"
            })
        );
        
        // Create a grant using the coffee batch as collateral
        uint256 grantAmount = 2000e6; // $2,000 USDC
        uint256 revenueShareBps = 2000; // 20% revenue share (in basis points)
        uint256 durationYears = 3; // 3 years grant duration
        ...
        uint256 grantId = grantManager.createGrant(
            ...
            grantAmount,
            batchIds,
            revenueShareBps, // 20% revenue share
            durationYears, // 3 years
            purpose
        );
        // Grant verification can be done by fetching the struct and checking fields if needed
        ICooperativeGrantManager.GrantInfo memory grantInfo = grantManager.getGrant(grantId);
        address cooperative = grantInfo.cooperative;
        uint256 amount = grantInfo.amount;

        console.log("Grant created - ID:", grantId);
        console.log("Cooperative:", cooperative);
        console.log("Amount:", amount);
        console.log("[PASS] Cooperative grant creation successful");
    }
    ...
    /**
     * @dev Test 6: Coffee purchase and grant repayment workflow
     */
    function testCoffeePurchaseAndGrantRepayment() public {
        console.log("=== TEST 6: COFFEE PURCHASE AND GRANT REPAYMENT ===");
        
        // Setup: Create batch and grant
        vm.prank(BOB);
        uint256 batchId = coffeeToken.createBatch(
            CoffeeStructs.BatchCreationParams({
                productionDate: block.timestamp - 2 days,
                expiryDate: block.timestamp + 150 days,
                quantity: 300,
                pricePerKg: 8e6,
                grantValue: 900e6,
                ipfsHash: "ipfs://batch3"
            })
        );
        
        uint256 grantAmount2 = 1500e6; // $1,500
        uint256[] memory batchIds = new uint256[](1);
        batchIds[0] = batchId;
        string memory purpose = "Pre-harvest financing";
        vm.prank(BOB);
        grantManager.createGrant(
            BOB,
            grantAmount2,
            batchIds,
        ...
        // Bob can now repay grant with coffee sales proceeds
        assertTrue(bobPayment >= grantAmount2, "Bob received enough to repay grant");
        
        console.log("Coffee purchased:", purchaseAmount, "kg");
        console.log("Total cost:", totalCost);
        console.log("Bob received:", bobPayment);
        console.log("Grant amount:", grantAmount2);
        console.log("[PASS] Coffee purchase and grant workflow successful");
    }
    ...
    vm.prank(DAVID);
    uint256 projectId = coffeeToken.createGreenfieldProject(
        "ipfs://greenfield1",
        block.timestamp + 30 days,  // plantingDate
        block.timestamp + 3 * 365 days,  // maturityDate
        2000,  // projectedYield
        10000e6  // grantValue
    );
    // Verify greenfield project creation
    // Check via individual calls to avoid struct name conflicts
    (bool isGreenfield,, , uint256 projectedYield,) = coffeeToken.getGreenfieldProjectDetails(projectId);
    assertTrue(isGreenfield, "Greenfield project should exist");
    assertEq(projectedYield, 2000, "Projected yield should match");
        
        // Create development grant for greenfield project
        uint256 grantAmount3 = 8000e6; // $8,000 for development
        uint256[] memory collateralTokenIds = new uint256[](1);
        uint256[] memory collateralAmounts = new uint256[](1);
        collateralTokenIds[0] = projectId;
        collateralAmounts[0] = 2000; // Future production as collateral
        ...
        grantManager.createGrant(
            ...
            grantAmount3,
            collateralTokenIds,
            1000, // 10% revenue share for development
            5, // 5 years for development
            "Greenfield coffee farm development"
        );
        
        console.log("Greenfield project created - ID:", projectId);
        console.log("Projected yield:", projectedYield, "kg/year");
    ...
    function testSystemStressWithCoffeeBatches() public {
        ...
        for (uint i = 0; i < numberOfBatches; i++) {
            ...
            uint256 grantValue = 1000e6 + (i * 100e6);
        ...
        // Create grants against multiple batches
        uint256 totalCollateralValue = 0;
        for (uint i = 0; i < numberOfBatches; i++) {
            uint256 batchId = 1 + i; // Batches start from ID 1
            // Check via individual calls to avoid struct name conflicts
            CoffeeStructs.BatchInfo memory batchInfo = coffeeToken.getBatchInfo(batchId);
            uint256 currentQuantity = batchInfo.currentQuantity;
            uint256 pricePerKg = batchInfo.pricePerKg;
            // No direct 'exists' field, so just sum if price > 0
            if (pricePerKg > 0) {
                totalCollateralValue += (currentQuantity * pricePerKg);
            }
        }
    ...
    function testComprehensiveWAGADAOSummary() public view {
        ...
        for (uint i = 1; i <= 15; i++) {
            // Check via individual call to avoid struct name conflicts
            try coffeeToken.getBatchInfo(i) returns (CoffeeStructs.BatchInfo memory batchInfo) {
                if (batchInfo.pricePerKg > 0) totalCoffeeBatches++;
            } catch {
                // Batch doesn't exist, skip
            }
        }
        ...
        console.log("* Cooperative grant management working");
